...
def test_fetch_returns_cached_when_not_overwrite(tmp_path, monkeypatch):
    logger = logging.getLogger("test_civic_fetch")
    ...
    def _fake_fetch(*, logger=None, **kwargs):
        called["count"] += 1
        return [{"id": 999}]
    ...
    out = civic_fetch.fetch_civic_evidence(
        oncogene=None, civic_path=raw, overwrite=False, limit=None, logger=logger
    )
...
def test_fetch_filters_and_limit(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *, logger=None, **kwargs: payload,
    )
    ...
    out = civic_fetch.fetch_civic_evidence(
        oncogene="ALK", civic_path=raw, overwrite=True, limit=1, logger=logger
    )
...
def test_fetch_overwrite_replaces_file(tmp_path, monkeypatch):
    ...
    out = civic_fetch.fetch_civic_evidence(
        oncogene=None, civic_path=raw, overwrite=True, limit=None, logger=logger
    )
