...
def help_request(
    url: str, headers: dict, payload: dict = None, method: str = "GET"
) -> dict:
    ...
    try:
        ...
        if "application/json" not in resp.headers.get("Content-Type", ""):
            raise RuntimeError(
                f"Expected JSON but got {resp.headers.get('Content-Type')} from {url}"
            )
...
def graphql_query(
    url: str, query: str, variables: dict | None = None, headers: dict | None = None
) -> dict[str, Any]:
