    ...
    merge into "alkfred"."public"."fact_evidence_assoc" as DBT_INTERNAL_DEST
        using "fact_evidence_assoc__dbt_tmp100316587183" as DBT_INTERNAL_SOURCE
        on (
    ...
    when matched then update set
        "eid" = DBT_INTERNAL_SOURCE."eid","doid" = DBT_INTERNAL_SOURCE."doid","molecular_profile_id" = DBT_INTERNAL_SOURCE."molecular_profile_id","status" = DBT_INTERNAL_SOURCE."status","evidence_type" = DBT_INTERNAL_SOURCE."evidence_type","direction" = DBT_INTERNAL_SOURCE."direction","significance" = DBT_INTERNAL_SOURCE."significance","pub_year" = DBT_INTERNAL_SOURCE."pub_year","ingestion_run_id" = DBT_INTERNAL_SOURCE."ingestion_run_id","ingested_at_utc" = DBT_INTERNAL_SOURCE."ingested_at_utc"
