  ...
  create  table "alkfred"."public"."fact_therapy_disease__dbt_tmp"
...
agg as (
    select
        ...
        count(distinct case when b.significance = 'RESISTANCE'
                            then b.variant_sk end)::int   as n_resistant_variants,
        count(distinct case when b.significance = 'SENSITIVITY'
                            then b.variant_sk end)::int   as n_sensitive_variants
    from base as b
    group by
        b.ncit_id,
        b.doid
